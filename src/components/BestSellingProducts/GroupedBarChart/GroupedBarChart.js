...
class GroupedBarChart extends Component {
    ...
    constructor(props) {
        ...
        const margin = { top: 5, right: 20, bottom: 20, left: 15 };
    ...
    xScale = () => {
        let volumes = this.props.data.map(d => d.volume);
        return d3.scaleLinear().domain([0, d3.max(volumes) * 1.15]).range([0, this.state.width])
    };

    yScale = () => {
        return d3.scaleBand().domain(d3.range(this.props.data.length)).range([0, this.state.height]).round(true).paddingInner(0.1);
    }

    yGroupScale = () => {
        return d3.scaleBand().domain(d3.range(this.props.data[0].values.length+1)).range([0, this.yScale().bandwidth()]).round(true).padding(0.025);
    }
    ...
    getXValue = (d) => { return d.volume };
    getYValue = (d) => {
        let volumes = this.props.data.map(v => v.volume).sort(d3.descending)
        return (volumes.indexOf(d.volume));
    };
    ...
    createChart = () => {
        ...
        // Plot
        this.scatter = this.svg.append('g').attr("class", "data-groupedbarchart").attr("clip-path", "url(#clip)").attr("transform", "translate(" + this.state.padding.left + "," + this.state.padding.top + ")");
        // Axis
    ...
    updateLegend = () => {
        ...
        let legendGroup = this.svg
            ...
            .on("mouseover", function (d, i) {
                d3.select('.data-groupedbarchart').selectAll("g.serie:not(.serie-" + i).transition().style("opacity", 0.1);
                //d3.selectAll('.legend').style("opacity", 0.5).style("text-decoration", "line-through");
            })
            .on("mouseout", function (d) {
                d3.select('.data-groupedbarchart').selectAll("g").transition().style("opacity", 1);
            })
    ...
    updateChart = () => {
        ...
        this.setState((oldState, oldProps) => (
            ...
            scatterSerie
                ...
                .ease(d3.easeLinear)
                .attr("transform", (d, i) => `translate(0,${scales.yScaleTransformed(this.getYValue(d))})`)
                ;

            // Serie Rect
            let scatterSerieRect = scatterSerie
                .selectAll("rect.global")
                .data(d => [d])
            ...
            scatterSerieRect
                ...
                .style("fill", (d, i) => this.fillColor("global-"+d.id))
                ...
                .attr("y", (d, i) => scales.yGroupScale(0))
            ...
            // Sub serie Rect
            let scatterSerieRectRest = scatterSerie
                .selectAll("rect.rest")
                .data(d => d.values)

                scatterSerieRectRest
                .enter()
                .append("rect")
                .attr("class", "rest")
                .merge(scatterSerieRectRest)
                .style("fill", (d, i) => this.fillColor("rest-"+d.id))
                .attr("stroke", (d) => '#fff')
                ...
                .ease(d3.easeLinear)
                .style("opacity", 0.8)
                .attr("height", scales.yGroupScale.bandwidth())
                .attr("y", (d, i) => scales.yGroupScale(i + 1))
                .attr("width", d => scales.xScaleTransformed(this.getXValue(d)) - scales.xScaleTransformed(0))
                ;

            scatterSerieRectRest.exit().remove();

            // Labels
            let scatterSerieGroupLabel = scatterSerie
                .selectAll("g.label.global")
                .data(d => [d]);

            let newScatterSerieGroupLabel = scatterSerieGroupLabel
                .enter()
                .append("g")
                .attr("class", "label global")
                ;
            ...
            scatterSerieGroupLabel
                ...
                .duration(this.animationDuration)
                .ease(d3.easeLinear)
                .attr("transform", d => `translate(${scales.xScaleTransformed(this.getXValue(d)) - scales.xScaleTransformed(0)},${scales.yGroupScale(0)})`)
            ;
            ...
            scatterSerieLabel
                ...
                .attr("fill", "#000")
                .attr("x", -30)
                .attr("y", scales.yGroupScale(0) + scales.yGroupScale.bandwidth()/2)
                .attr("dy", "+0.1em")
                ;*/
            ...
            newScatterSerieLabelNumber
                ...
                .tween("text", function (d) {
                    ...
                    var end = d.volume;                     // specified end value
                    var interpolator = d3.interpolateNumber(start, end); // d3 interpolator
                    selection.attr("value", end);
                    return function (t) { selection.text(d.label + "   " + component.formatCurrency(interpolator(t))); };  // return value
                })
                ;


            // Sub-serie Labels
            let scatterSerieGroupLabelRest = scatterSerie
                .selectAll("g.label.rest")
                .data(d => d.values);

            scatterSerieGroupLabelRest.exit().remove();

            let newScatterSerieGroupLabelRest = scatterSerieGroupLabelRest
                .enter()
                .append("g")
                .attr("class", "label rest")
                ;

            scatterSerieGroupLabelRest = newScatterSerieGroupLabelRest.merge(scatterSerieGroupLabelRest);

            scatterSerieGroupLabelRest
                .transition()
                .duration(this.animationDuration)
                .ease(d3.easeLinear)
                .attr("transform", (d, i) => `translate(${scales.xScaleTransformed(this.getXValue(d)) - scales.xScaleTransformed(0)},${scales.yGroupScale(i+1)})`)
            ;

            let scatterSerieLabelNumberRest = scatterSerieGroupLabelRest
                .selectAll("text.number")
                .data(d => [d]);

            let newScatterSerieLabelNumberRest = scatterSerieLabelNumberRest
                .enter()
                .append("text")
                .attr("class", "number")
                .attr("value", 0)
            ...
            newScatterSerieLabelNumberRest
                .merge(scatterSerieLabelNumberRest)
                .transition()
                .duration(this.animationDuration)
                .ease(d3.easeLinear)
                .attr("font-size", 4)
    ...
    render() {
        ...
        return (
            <div id="bestSellingProductsChart" ref={el => this.divEl = el}>
