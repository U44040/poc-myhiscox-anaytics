...
import * as DataGenerator from '../../utils/BestSellingProductsDataGenerator';
...
import GroupedBarChart from './../../components/BestSellingProducts/GroupedBarChart/GroupedBarChart';
...
class BestSellingProducts extends Component {
  ...
  getSegmentedData = (data) => {
    ...
    let differentProductVariants = {};
    for (let d of data) {
      for (let project of d.projects) {
        for (let pv of project.productVariants) {
          differentProductVariants[pv.idProductVariant] = {
            idProductVariant: pv.idProductVariant,
            name: pv.name,
            product: pv.product,
            data: [],
          }
    ...
    differentProductVariants = Object.values(differentProductVariants);

    let series = differentProductVariants.map(d => {
      let values = deepClone(data);
      for (let value of values) {
        value.projects = value.projects.map(p => (
          {
            ...p,
            productVariants: p.productVariants.filter(pv => pv.idProductVariant == d.idProductVariant)
          }
        ))
        value.projects = value.projects.filter(p => p.productVariants.length > 0);
      }
      return {
        id: d.idProductVariant,
        label: d.name,
        values: values,
      };
    });
    ...
    if (this.state.segmentType) {
      ...
      for (let serie of series) {
        ...
        let segmentedSeries = differentValues.map(d => {
          let values = deepClone(serie.values);
          for (let value of values) {
            switch (this.state.segmentType) {
              case 'network':
  ...
  getAggregatedData = (data) => {
    ...
    for (let serie of series) {
      ...
      let aggregatedSerie = {
        ...serie,
        values: serie.values.map(d => {
          let volume = 0;
          for (let val of d.values) {
            volume += val.volume;
          }
          serieVolume += volume;
          return {
      ...
      //console.log(aggregatedSerie);

      if (Array.isArray(aggregatedSerie.values)) {
        if (aggregatedSerie.values[0].label == "Total") {
          aggregatedSerie.values = [];
        }
      }
  ...
  render = () => {
    let chart = '';
    if (this.state.aggregatedData && this.state.aggregatedData.length > 0) {
      chart = <GroupedBarChart segmentType={this.state.segmentType} data={this.state.aggregatedData} />
    }
    return (
